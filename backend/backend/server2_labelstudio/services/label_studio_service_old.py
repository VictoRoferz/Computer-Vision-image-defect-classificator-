...
class LabelStudioService:
    ...
    """def _setup_local_storage(self, project_id: int) -> None:
        ...
        Storage configuration API for label studio 
    ...
    def _setup_local_storage(self, project_id: int) -> None:
        try:    
            ...
            storage = self.client.import_storage.local.create(
                project=project_id,
                path="/data",  # Current directory (which is /data/images),
                use_blob_urls=False,
                regex_filter=".*\\.(jpg|jpeg|png)$"
            )
            logger.info(f"Local storage configured for porject {project_id}, ID ={storage.id}")
        except Exception as e:
