...
@router.get("/images/serve/{category:path}")
async def serve_image(category: str):
    """
    Serve an image file by its storage path.

    Args:
        category: Path relative to /data, e.g. 'unlabeled/ab/cd/.../file.jpg'
                  or 'labeled/file.jpg'

    Returns:
        The image file
    """
    image_path = settings.data_root / category
    if not image_path.is_file():
        raise HTTPException(status_code=404, detail="Image not found")

    # Security: ensure the resolved path is within data_root
    try:
        image_path.resolve().relative_to(settings.data_root.resolve())
    except ValueError:
